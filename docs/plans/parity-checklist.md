...
| K04 | Apple Health sync (Tiimo Wellbeing) | K | ios | 8B, R11 | partial | 0.5 | SHIPPED in Round 11: explicit device-local opt-in plus idempotent, write-only mindful-minute samples after successful focus completion. Unit tests, a 16-test simulator UI suite, and a generic arm64 device build are green. REMAINS: signed physical-iPhone authorization/sample proof and the separately scoped sleep-schedule read for wind-down suggestions. |
