        ...
        ActivityConfiguration(for: FocusAttributes.self) { context in
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text(context.attributes.title)
                        ...
                        .lineLimit(1)
                    Text(context.state.paused ? "paused" : "focusing · \(context.attributes.targetMin) min target")
            ...
            DynamicIsland {
                ...
                DynamicIslandExpandedRegion(.bottom) {
                    Text(context.state.paused ? "Paused — the ring will wait." : "Deep in it. The app has the controls.")
                        .font(.system(size: 12, weight: .medium))
                        .opacity(0.75)
                }
            } compactLeading: {
