  ...
  return db.transaction(async (tx) => {
    ...
    const [series] = await tdb
      ...
      .values({
        ...
        title: input.title,
        emoji: input.emoji ?? task.emoji ?? null,
        categoryId: input.categoryId ?? task.categoryId ?? null,
        durationMin: input.durationMin,
        checklistTemplate,
        energy: input.energy !== undefined ? input.energy : (task.energy ?? null),
        priority: input.priority ?? task.priority ?? "none",
        tags: input.tags !== undefined ? input.tags : null,
        notes: input.notes !== undefined ? input.notes : (task.notes ?? null),
