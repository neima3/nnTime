 ...
 *
 * Runs once per process (guarded by a module-level promise) and serializes
 * independent processes with a PostgreSQL advisory lock. If DATABASE_URL is
...
async function runMigrations(): Promise<void> {
  ...
  await runMigrationsForUrl(url, drizzleDir);
}

export async function runMigrationsForUrl(
  url: string,
  drizzleDir: string,
): Promise<void> {
  const sql = postgres(url, { max: 1, connect_timeout: 5 });
  try {
    await sql`SELECT pg_advisory_lock(hashtextextended('kairo-schema-migrations', 0))`;
    try {
      ...
      await sql`SELECT pg_advisory_unlock(hashtextextended('kairo-schema-migrations', 0))`;
    }
  } finally {
